...
export default function AdminSettings() {
  ...
  const handleSave = () => {
    try {
      localStorage.setItem("cmr_admin_settings", JSON.stringify({ notifications, privacy }));
    } catch {}
    setSaved(true);
  ...
  const Toggle = ({ value, onChange }) => (
    <button
      ...
      className={`relative w-11 h-6 rounded-full transition-colors ${value ? "bg-yellow-400" : "bg-gray-300"}`}
    >
  ...
  return (
    <div className="min-h-screen flex flex-col bg-neutral-900">
      ...
      <div className="px-8 pt-2 pb-1">
        <span className="text-gray-400 text-xs tracking-widest uppercase">Admin Settings</span>
      </div>
      <div className="flex flex-1">
        <AdminSidebar active="settings" />
        <main className="flex-1 p-6 bg-white rounded-tl-xl overflow-auto">
          <div className="max-w-lg mx-auto">
            <h2 className="text-gray-900 font-extrabold text-xl mb-6">Settings</h2>
            ...
            <div className="bg-gray-50 rounded-xl p-6 border border-gray-200 mb-4">
              <h3 className="text-gray-800 font-bold text-sm mb-4">Notification Preferences</h3>
              <div className="space-y-4">
                {[
                  { key: "newBooking", label: "New Booking Alerts", desc: "Get notified when a new booking is made" },
                  { key: "statusChange", label: "Status Change Alerts", desc: "Get notified when booking status changes" },
                  { key: "cancellation", label: "Cancellation Alerts", desc: "Get notified when a booking is cancelled" },
                ].map(({ key, label, desc }) => (
            ...
            {/* Privacy Settings */}
            <div className="bg-gray-50 rounded-xl p-6 border border-gray-200 mb-4">
              <h3 className="text-gray-800 font-bold text-sm mb-4">Privacy & Security</h3>
              <div className="space-y-4">
                {[
                  { key: "showEmail", label: "Show Email Publicly", desc: "Display your email on the admin panel" },
            ...
            <div className="bg-red-50 rounded-xl p-6 border border-red-200 mb-6">
              <h3 className="text-red-700 font-bold text-sm mb-4">Danger Zone</h3>
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-gray-800 text-sm font-medium">Clear All Bookings</p>
                  <p className="text-gray-400 text-xs">Permanently delete all booking data</p>
                </div>
                <button
                  onClick={() => { if (confirm("Are you sure? This cannot be undone.")) { localStorage.removeItem("cmr_bookings"); localStorage.removeItem("cmr_deleted_bookings"); localStorage.removeItem("cmr_booking_statuses"); } }}
            ...
            <div className="flex gap-3">
              <button onClick={handleSave} className="flex-1 px-6 py-3 bg-yellow-400 hover:bg-yellow-500 text-black font-bold rounded-lg text-sm transition-colors">
                Save Settings
              </button>
              <Link href="/admin/profile" className="flex-1 flex items-center justify-center px-6 py-3 bg-black hover:bg-gray-800 text-white font-bold rounded-lg text-sm transition-colors">
                Go to Profile
              </Link>
            </div>
          </div>
