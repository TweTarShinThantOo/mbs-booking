...
// ✅ Hardcoded admin emails — only these can access admin panel
// Add your real admin email(s) here
...
export default function AdminLoginPage() {
  ...
  try {
    ...
    if (error) { setLoginErrors({ email: "Invalid email or password." }); return; }
    ...
    if (!profile || profile.role !== "admin") {
      ...
      setLoginErrors({ email: "This account does not have admin access." });
      return;
    }
  ...
  // ✅ Real Supabase signup for admin
  ...
  try {
    ...
    if (error) { setSignupErrors({ email: error.message }); return; }

    // ✅ Save to public.users with role: "admin"
...
console.log("insert error:", JSON.stringify(insertError)); // ✅ add this
console.log("user id:", data.user.id); // ✅ add this
      ...
      <div className="w-full max-w-md bg-white rounded-xl shadow-2xl overflow-hidden">
        ...
        <div className="px-8 py-8">
          ...
          <div className="flex items-center gap-2 bg-yellow-50 border border-yellow-300 rounded-lg px-4 py-2 mb-6">
            <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 text-yellow-600 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {tab === "login" && (
            <div>
              ...
              <div className="space-y-4">
                ...
                >
                  {loginLoading ? "Logging in..." : "Log In to Admin Panel"}
                </button>
